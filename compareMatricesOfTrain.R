...
data_analysis_long %>% filter(name == "t") %>%
  group_by(o, d, case) %>% summarise(value = sum(value)) %>%
...
# data_analysis_long_ %>% filter(name == "t", !is.na(BBSR_type), !is.na(BBSR_type_d)) %>%
...
# 
# ggsave(filename = "tmp/travel_time_scenario_1_area_type.png", device = png(),
#        width = 15, height= 10, units = "cm", scale = 1.5)
...
p = summary %>% filter(name== "t") %>%
  filter(!is.na(BBSR_type), !is.na(BBSR_type_d)) %>%
  ggplot(aes(x=case, y = value, fill = case)) + geom_bar(color = "black", stat = "identity", position = "dodge") + 
  facet_grid(BBSR_type~BBSR_type_d) + theme_bw() + xlab("Scenario") + ylab("Average travel time (h)")
...
q = summary %>% filter(name== "t") %>%
  filter(!is.na(BBSR_type), !is.na(BBSR_type_d)) %>%
  ggplot(aes(x=case, y = dist/1000, fill = case)) + geom_bar(color = "black", stat = "identity", position = "dodge") + 
  facet_grid(BBSR_type~BBSR_type_d) + theme_bw() + xlab("Scenario") + ylab("Average travel distance (km)")

q

ggsave(q, filename = "tmp/avg_travel_distance_scenario_1_area_type.jpeg", device = "jpeg",
       width = 15, height= 10, units = "cm", scale = 1.5)
