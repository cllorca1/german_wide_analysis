...
my_folder = "Y:/projects/2019/BASt/data/input_files_LD/germanymodel/microData/2030/"

pp_2030 <- read_csv(paste(my_folder, "pp0_2030.csv", sep = ""))
pp_new <- pp_2030
jj_2030 <- read_csv(paste(my_folder, "jj0_2030.csv", sep = ""))
jj_new <- jj_2030[,c(1:3,5,6)] # exclude column 4 which is job type - do not need it duplicated
...
write_csv(pp_new, paste(my_folder, "pp_mito_2030.csv", sep = ""))


hh_2030 <- read_csv(paste(my_folder, "hh0_2030.csv", sep = ""))
dd_2030 <- read_csv(paste(my_folder, "dd0_2030.csv", sep = ""))
dd_new <- dd_2030[,c(1, 4,5)]
hh_new <- hh_2030 %>% left_join (dd_new, by = c("id"))
hh_new <- hh_new[,c(1,3:7)]

write_csv(hh_new, paste(my_folder, "hh_mito_2030.csv", sep = ""))
# 
# ## sampling 1 percent
